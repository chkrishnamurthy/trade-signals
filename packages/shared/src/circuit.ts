 ...
 *
 * Every broker API this product talks to sits behind an edge (Cloudflare or
 * similar) that enforces its own limits *in addition to* the documented API
 * limits, and does so per path. Observed on Fyers, 2026-08-22: `/data/quotes`
 * returned `HTTP 429` with a `text/plain` body of `error code: 1015` and
 * `Retry-After: 1358` — a fixed ~22 minute ban — while `/data/marketStatus`
 * and `/data/history` kept answering 200 throughout.
 *
 ...
 *
 * Provider-neutral: lives in `@equitywise/shared` so every provider package
 * shares one implementation without importing a sibling provider.
 */
