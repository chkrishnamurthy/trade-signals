...
const POOLED = 'postgresql://u:p@db-pooler.example.com/db?sslmode=require';
const DIRECT = 'postgresql://u:p@db.example.com/db?sslmode=require';
...
describe('readDatabaseEnv', () => {
  ...
  it('accepts the postgres:// scheme as well as postgresql://', () => {
    const url = 'postgres://u:p@db.example.com/db?sslmode=require';
...
describe('isPooledUrl', () => {
  it('detects pooled endpoints by the -pooler host suffix', () => {
  ...
  it('looks at the host, not the database name or query string', () => {
    expect(isPooledUrl('postgresql://u:p@db.example.com/my-pooler-db')).toBe(false);
  });
});
