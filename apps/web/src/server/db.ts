 ...
 * until Postgres refused new ones.
 *
...
export function getDatabase(): Database {
  ...
  const handle = createDatabase({
    // Modest: a route handler holds a connection for milliseconds.
    max: 5,
    // Idle connections get dropped; without a handler that is an unhandled
